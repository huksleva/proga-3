# Скрипт Python. Инструкция для macOS
...
python3 --version
```  
Если видите версию Python, переходите к шагу 2.  
Если пишет `"command not found"`, скачайте Python с [python.org](https://www.python.org).
...
### Шаг 3: Создание файла установки  
Создайте файл `install.py`:  
```bash
nano install.py
```  


Скопируйте этот код и вставьте в файл `install.py`:
...
После установки активируйте окружение и проверьте пакеты:  
```bash
source venv/bin/activate
pip list
```

Проверка каждого пакета отдельно:  
```bash
pip show mypy
pip show pylint
pip show flake8
...
```  
Если видите версию Python, переходите к шагу 2.  
Если пишет `"не является внутренней или внешней командой..."`, скачайте Python с [python.org](https://www.python.org) и **обязательно отметьте галочку “Add Python to PATH”** при установке.
...
notepad install.py
```  
Скопируйте туда **тот же самый код**, что указан выше для macOS (он универсален и работает на Windows без изменений).
...
Если все инструменты установлены правильно, вы должны увидеть в выводе строки, содержащие:  
- Версию `mypy`  
- Версию `pylint`  
- Версию `flake8`  
- Версию `black`  
...
Если какой-то инструмент не появляется в списке — установите его вручную внутри активированного виртуального окружения:  
```bash
pip install имя_инструмента
```

---


Работу выполнили: Тоц Л. А. и Калинин И. В.
Группа: ИВТ-2